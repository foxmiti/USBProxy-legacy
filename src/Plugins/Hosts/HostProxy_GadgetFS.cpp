...
int HostProxy_GadgetFS::connect(Device *device, int timeout)
{
	...
	if (p_is_connected)
	{
		fprintf(stderr, "GadgetFS already connected.\n");
		return 0;
	}

	if (generate_descriptor(device) != 0)
	{
		return 1;
	}
...
void HostProxy_GadgetFS::disconnect()
{
	if (!p_is_connected)
	{
		fprintf(stderr, "GadgetFS not connected.\n");
		return;
	}
...
int HostProxy_GadgetFS::control_request(usb_ctrlrequest *setup_packet, int *nbytes, __u8 **dataptr, int timeout)
{
	...
	if (debugLevel > 1)
		fprintf(stderr, "gadgetfs: %d events received\n", nevent);

	for (i = 0; i < nevent; i++)
	{
		if (debugLevel > 0 && events[i].type != GADGETFS_SETUP)
			fprintf(stderr, "gadgetfs: event %d\n", events[i].type);
		...
		case GADGETFS_SETUP:
			...
			if (lastControl.bRequest == USB_REQ_SET_CONFIGURATION && !(lastControl.bRequestType & 0x80))
			{
				control_ack();
			}
...
{
	if (debugLevel)
		fprintf(stderr, "Stalling EP%02x\n", endpoint);
	if (endpoint)
	{
